/**
 * NO5 ナレーション合成 → output/dx-award-movie-no5-voice.mp4
 */
...
const WORK_DIR = path.join(OUTPUT_DIR, "no5-voice-work");
const INPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no5.mp4");
const MERGED_WAV = path.join(WORK_DIR, "no5-narration-merged.wav");
const CONCAT_LIST = path.join(WORK_DIR, "concat.txt");
const OUTPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no5-voice.mp4");
...
function buildSegmentsFromConstants() {
  if (NO5_NARRATION_SEGMENTS.length !== 2) {
    throw new Error("no5-narration-segments.mjs は 2 文である必要があります。");
  }
  const ms = [NO5_PROFILE_SCENE_MS, NO5_AFTER_SCENE_MS];
  return NO5_NARRATION_SEGMENTS.map((text, i) => ({
    text,
...
async function main() {
  ...
  if (Math.abs(videoDur - totalSec) > 0.25) {
    console.warn(
      `注意: 動画 ${videoDur}s と音声合計 ${totalSec}s がずれています。prep:no5-voice-timeline 後に render:no5 を再実行したか確認してください。`,
    );
  }
