/**
 * NO7 ナレーション合成 → output/dx-award-movie-no7-voice.mp4
 */
import { spawn } from "node:child_process";
import fs from "node:fs/promises";
import path from "node:path";
import { NO7_AFTER_SCENE_MS, NO7_PROFILE_SCENE_MS } from "../no7-video.constants.mjs";
import { NO7_NARRATION_SEGMENTS } from "./no7-narration-segments.mjs";
import { readMp4DurationSeconds } from "./read-mp4-duration.mjs";

const ROOT = process.cwd();
const OUTPUT_DIR = path.join(ROOT, "output");
const WORK_DIR = path.join(OUTPUT_DIR, "no7-voice-work");
const INPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no7.mp4");
const MERGED_WAV = path.join(WORK_DIR, "no7-narration-merged.wav");
const CONCAT_LIST = path.join(WORK_DIR, "concat.txt");
const OUTPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no7-voice.mp4");

const EDGE_VOICE = process.env.NO7_NARRATION_VOICE ?? "ja-JP-NanamiNeural";
const EDGE_RATE = process.env.NO7_VOICE_RATE ?? "+18%";

function buildSegmentsFromConstants() {
  if (NO7_NARRATION_SEGMENTS.length !== 2) {
    throw new Error("no7-narration-segments.mjs は 2 文である必要があります。");
  }
  const ms = [NO7_PROFILE_SCENE_MS, NO7_AFTER_SCENE_MS];
  return NO7_NARRATION_SEGMENTS.map((text, i) => ({
    text,
