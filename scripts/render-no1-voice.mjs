/**
 * NO1 用ナレーションを no1-video.constants.mjs の各シーン秒に合わせて合成する。
 * 台本は scripts/no1-narration-segments.mjs。尺は `npm run prep:no1-voice-timeline` で更新。
 *
 * 前提: pip install edge-tts、インターネット接続
 *
 * 環境変数:
 *   NO1_NARRATION_VOICE  既定 ja-JP-NanamiNeural（女性）。男性は ja-JP-KeitaNeural のみ（日本語はこの2種）
 *   NO1_VOICE_RATE       既定 +28%（速め。+20%〜+35% で微調整）
 */
...
const WORK_DIR = path.join(OUTPUT_DIR, "no1-voice-work");
const INPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no1.mp4");
const MERGED_WAV = path.join(WORK_DIR, "no1-narration-merged.wav");
const CONCAT_LIST = path.join(WORK_DIR, "concat.txt");
const OUTPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no1-voice.mp4");
...
function buildSegmentsFromConstants() {
  ...
  if (!Array.isArray(ticks) || ticks.length !== 4) {
    throw new Error("no1-video.constants.mjs の NO1_DASHBOARD_TICK_MS は長さ4の配列である必要があります。");
  }
  if (NO1_NARRATION_SEGMENTS.length !== 7 && NO1_NARRATION_SEGMENTS.length !== 8) {
    throw new Error("no1-narration-segments.mjs は 7 文（アウトロなし）または 8 文（アウトロあり）である必要があります。");
  }
  const baseMs = [
    NO1_PROFILE_SCENE_MS,
    NO1_BEFORE_SCENE_MS,
  ...
  const ms = NO1_NARRATION_SEGMENTS.length === 8 ? [...baseMs, NO1_OUTRO_SCENE_MS] : baseMs;
  return NO1_NARRATION_SEGMENTS.map((text, i) => ({
    text,
...
/** 各セグメントを正確に sec 秒に揃える（長い場合は切る、短い場合は無音で埋める） */
...
async function main() {
  ...
  if (Math.abs(videoDur - totalSec) > 0.25) {
    console.warn(
      `注意: 動画の長さ ${videoDur}s とセグメント合計 ${totalSec}s が一致しません。prep:no1-voice-timeline の後に render:no1 を再実行したか確認してください。`,
    );
  }
