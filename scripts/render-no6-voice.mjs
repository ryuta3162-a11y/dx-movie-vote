/**
 * NO6 ナレーション合成 → output/dx-award-movie-no6-voice.mp4
 */
import { spawn } from "node:child_process";
import fs from "node:fs/promises";
import path from "node:path";
import { NO6_AFTER_SCENE_MS, NO6_PROFILE_SCENE_MS } from "../no6-video.constants.mjs";
import { NO6_NARRATION_SEGMENTS } from "./no6-narration-segments.mjs";
import { readMp4DurationSeconds } from "./read-mp4-duration.mjs";

const ROOT = process.cwd();
const OUTPUT_DIR = path.join(ROOT, "output");
const WORK_DIR = path.join(OUTPUT_DIR, "no6-voice-work");
const INPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no6.mp4");
const MERGED_WAV = path.join(WORK_DIR, "no6-narration-merged.wav");
const CONCAT_LIST = path.join(WORK_DIR, "concat.txt");
const OUTPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no6-voice.mp4");

const EDGE_VOICE = process.env.NO6_NARRATION_VOICE ?? "ja-JP-NanamiNeural";
const EDGE_RATE = process.env.NO6_VOICE_RATE ?? "+18%";

function buildSegmentsFromConstants() {
  if (NO6_NARRATION_SEGMENTS.length !== 2) {
    throw new Error("no6-narration-segments.mjs は 2 文である必要があります。");
  }
  const ms = [NO6_PROFILE_SCENE_MS, NO6_AFTER_SCENE_MS];
  return NO6_NARRATION_SEGMENTS.map((text, i) => ({
    text,
...
async function main() {
  ...
  if (Math.abs(videoDur - totalSec) > 0.25) {
    console.warn(`注意: 動画 ${videoDur}s と音声合計 ${totalSec}s がずれています。`);
  }
  ...
  await runCommand(ffmpeg, ["-y", "-f", "concat", "-safe", "0", "-i", CONCAT_LIST, "-c", "copy", MERGED_WAV], { shell: false });
  ...
  const videoCodec = needsVideoPad ? ["-c:v", "libx264", "-preset", "fast", "-crf", "18", "-pix_fmt", "yuv420p"] : ["-c:v", "copy"];
