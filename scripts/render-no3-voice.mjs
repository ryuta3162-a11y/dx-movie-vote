/**
 * NO3 用ナレーションを no3-video.constants.mjs の各シーン秒に合わせて合成する。
 * 台本は scripts/no3-narration-segments.mjs（4文）。
 *
 * 環境変数:
...
const WORK_DIR = path.join(OUTPUT_DIR, "no3-voice-work");
const INPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no3.mp4");
const MERGED_WAV = path.join(WORK_DIR, "no3-narration-merged.wav");
const CONCAT_LIST = path.join(WORK_DIR, "concat.txt");
const OUTPUT_VIDEO = path.join(OUTPUT_DIR, "dx-award-movie-no3-voice.mp4");

const EDGE_VOICE = process.env.NO3_NARRATION_VOICE ?? "ja-JP-NanamiNeural";
const EDGE_RATE = process.env.NO3_VOICE_RATE ?? "+18%";

function buildSegmentsFromConstants() {
  if (NO3_NARRATION_SEGMENTS.length !== 4) {
    throw new Error("no3-narration-segments.mjs は 4 文である必要があります。");
  }
  const ms = [NO3_PROFILE_SCENE_MS, NO3_BEFORE_SCENE_MS, NO3_TRANSITION_SCENE_MS, NO3_AFTER_SCENE_MS];
  return NO3_NARRATION_SEGMENTS.map((text, i) => ({
    text,
...
async function main() {
  ...
  if (Math.abs(videoDur - totalSec) > 0.25) {
    console.warn(
      `注意: 動画の長さ ${videoDur}s とセグメント合計 ${totalSec}s が一致しません。prep:no3-voice-timeline の後に render:no3 を再実行したか確認してください。`,
    );
  }
